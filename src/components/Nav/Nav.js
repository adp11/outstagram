  ...
  return (
    <div className={`Nav ${(isSearchChatActive || isFullImageActive) ? "blur opac" : ""}`}>
      <div className="nav-container">
        <div onClick={refresh}>
          <img src={`${window.location.origin}/images/${darkMode ? "header-dark.png" : "header-light.png"}`} alt="Instagram" className="header" />
        </div>
        <SearchBox />
        <div className="nav-buttons">
          {(window.location.pathname === "/") ? (
            <svg onClick={refresh} aria-label="Home" color="currentColor" fill="currentColor" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M22 23h-6.001a1 1 0 01-1-1v-5.455a2.997 2.997 0 10-5.993 0V22a1 1 0 01-1 1H2a1 1 0 01-1-1V11.543a1.002 1.002 0 01.31-.724l10-9.543a1.001 1.001 0 011.38 0l10 9.543a1.002 1.002 0 01.31.724V22a1 1 0 01-1 1z" /></svg>
          ) : (
            <svg onClick={refresh} aria-label="Home" color="currentColor" fill="currentColor" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M9.005 16.545a2.997 2.997 0 012.997-2.997h0A2.997 2.997 0 0115 16.545V22h7V11.543L12 2 2 11.543V22h7.005z" fill="none" stroke="currentColor" strokeLinejoin="round" strokeWidth="2" /></svg>
          )}

          {(/^\/chat/.test(window.location.pathname)) ? (
            <Link to="/chat">
              <svg aria-label="Messenger" color="currentColor" fill="currentColor" height="24" role="img" viewBox="0 0 24 24" width="24"><path d="M12.003 1.131a10.487 10.487 0 00-10.87 10.57 10.194 10.194 0 003.412 7.771l.054 1.78a1.67 1.67 0 002.342 1.476l1.935-.872a11.767 11.767 0 003.127.416 10.488 10.488 0 0010.87-10.57 10.487 10.487 0 00-10.87-10.57zm5.786 9.001l-2.566 3.983a1.577 1.577 0 01-2.278.42l-2.452-1.84a.63.63 0 00-.759.002l-2.556 2.049a.659.659 0 01-.96-.874L8.783 9.89a1.576 1.576 0 012.277-.42l2.453 1.84a.63.63 0 00.758-.003l2.556-2.05a.659.659 0 01.961.874z" /></svg>

            </Link>
          ) : (
            <Link to="/chat">
              {/* eslint-disable-next-line */}
              <div style={{ position: "relative" }} onClick={handleCloseChatNotifications}>
                ...
                {(userData && userData.totalChatNotifs > 0) && (
                <div
                ...
                >
                  {userData.totalChatNotifs > 9 ? "9+" : `${userData.totalChatNotifs}`}
                </div>
                )}
              </div>
            </Link>
